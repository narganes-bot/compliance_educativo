...
// Reduce un nombre a una forma breve para minimizar datos (RGPD), pero RESPETA
// lo que el usuario ya ha escrito como iniciales. Solo reduce a iniciales cuando
// detecta un nombre completo de verdad (varias palabras con longitud > 2).
function toAlias(name) {
  if (!name) return "";
  const s = String(name).trim().replace(/\s+/g, " ");
  if (!s) return "";
  // Ya parece iniciales (contiene puntos) o es corto: se respeta tal cual.
  if (s.includes(".") || s.length <= 8) return s.slice(0, 40);
  const words = s.split(" ");
  // Varias palabras "de verdad" (nombre y apellidos): se reduce a iniciales.
  if (words.length >= 2 && words.some((w) => w.length > 2)) {
    return words.slice(0, 4).map((w) => w.charAt(0).toUpperCase()).join(". ") + ".";
  }
  // Una sola palabra (p. ej. un nombre de pila): se deja tal cual, acotada.
  return s.slice(0, 40);
}
